...
레이어 패널은 `영토·구역`, `인문 분포`, `지형지물`로 구성됩니다. 강, 호수, 사용자 지형지물 폴더를 펼쳐 개별 항목을 표시하거나 숨길 수 있으며, 국가명과 지명 표시는 `보기 > 이름 표시`에서 조절합니다. 권역과 행정구역은 국가별로 묶이고 행정구역은 다시 단계별로 표시됩니다. 내장 수계는 잠겨 있으며 선택 후 `편집용 복사 만들기`를 사용하면 원본을 숨기고 같은 형상의 편집 객체를 만들 수 있습니다. 개별 표시 상태와 폴더 열림 상태는 자동저장 및 GeoPackage의 `pandolab_project_settings`에 보존됩니다.
