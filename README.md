...
작업공간은 화면 너비에 따라 세 단계로 전환됩니다. 넓음·중간 화면에서는 상단 `보기`, 지도 하단의 추가·검색 작업바와 별도 편집창을 사용합니다. `799px` 이하에서는 하단의 `추가 / 검색 / 편집 / 보기` 탐색으로 같은 DOM의 논모달 바텀시트를 엽니다.
...
객체는 지도 또는 하단 검색으로 찾고 선택합니다. 국가·하위단위·지방·분포·지명·강·호수·지형의 표시 여부와 세부 표현은 `보기`에서 조절합니다. 좁은 화면에서는 같은 설정이 `보기` 시트에 표시되며 설정값과 컨트롤을 복제하지 않습니다. 내장 수계는 잠겨 있으며 선택 후 `편집용 복사 만들기`를 사용하면 원본을 숨기고 같은 형상의 편집 객체를 만들 수 있습니다. 표시 상태는 자동저장 및 GeoPackage의 `pandolab_project_settings`에 보존됩니다.
