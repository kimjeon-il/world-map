...
v0.22.0에서는 모바일의 지도·추가·편집 패널을 하나의 논모달 바텀시트 체계로 통합했습니다. 시트는 화면 하단과 좌우에 빈틈 없이 붙고 35%·60%·90% 높이로 조절할 수 있으며, 열린 상태에서도 지도 이동·확대·객체 선택이 가능합니다. 시트 높이가 바뀌어도 지도 카메라와 플로팅 도구 위치는 유지됩니다.
