...
작업공간은 화면 너비에 따라 세 단계로 전환됩니다. `1360px` 이상에서는 레이어 패널을 고정하고 편집 패널을 접을 수 있으며, `800~1359px`에서는 `레이어 / 추가 / 편집` 세로 레일과 모델리스 서랍을 사용합니다. `799px` 이하에서는 같은 탐색 순서를 하단에 표시하고 레이어·추가·편집을 논모달 바텀시트로 엽니다.
...
v0.18.2에서는 지도 위의 독립 `현재 작업` 카드를 제거하고 작업명·현재 단계·짧은 안내·완료 버튼을 상단 작업 도크에 통합했습니다. 대상 선택 개수는 긴 안내문 대신 완료·합병 버튼에 표시합니다.

v0.19.0에서는 국가·지명·지형지물·수계 편집창을 하나의 공용 셸과 컴포넌트 체계로 통합했습니다. 빈 상태, 정보 카드, 작업 버튼, 상세 정보와 삭제 영역이 객체 종류와 화면 크기에 관계없이 같은 구조와 여백을 사용합니다.
...
v0.30.0에서는 시작 과정을 미리보기·편집 가능·고화질 지도의 세 단계로 분리했습니다. 국가 원본이 준비되면 GPU 메시를 기다리지 않고 편집을 시작할 수 있으며, 기기·네트워크에 따라 원본 요청을 병렬 또는 순차 처리합니다. 시작 자산은 스트리밍으로 압축을 풀고 버전별 Cache Storage에 보관하며, 손상된 항목만 지운 뒤 자동으로 다시 받습니다.
...
v0.25.0에서는 국가·지역·행정구역·역사·지리 지역을 `TerritorialUnit` 공통 API로 조회·선택·편집합니다. 직접 상위 영역(`parentId`)과 최종 주권국(`sovereignId`)을 분리하고, 일반 영역 연산은 공통 geometry 커널을 사용합니다. 현재 화면의 `지역`은 내부적으로 `territory`, 역사·지리적 지역은 독립 `region` 유형이며 기존 `countryRegions` 프로젝트는 불러올 때 새 모델로 변환됩니다.

v0.24.0에서는 레이어 하위 항목의 장식 아이콘과 중복 `국명` 문구를 제거하고, 각 항목 오른쪽에 실행취소 가능한 삭제 버튼을 추가했습니다. 지역·행정구역은 범용 지형지물과 분리된 국가 영역으로 관리하며 국가별·행정 단계별 레이어, 전용 편집창, 나누기·합치기·영역 다시 지정·국가 이전·독립 작업을 제공합니다. 국가 Polygon은 국경과 해안선의 단일 기준이고, 명시적 구분을 시작한 부모의 남는 면적은 미지정 영역으로 보존됩니다. 이 관계는 프로젝트 상태와 GeoJSON·GeoPackage 입출력에 포함됩니다.

v0.23.0에서는 전역 탐색을 `레이어 / 추가 / 편집`으로 통일했습니다. Wide에서는 고정 레이어 패널과 접을 수 있는 모델리스 편집 패널, Compact에서는 세로 탐색 레일과 서랍, Mobile에서는 35%·60%·90% 논모달 바텀시트를 사용합니다. 패널을 열고 닫거나 시트 높이를 바꿔도 지도 중심·회전·배율과 플로팅 지도 도구 위치는 유지됩니다. 레이어 검색은 국가 코드도 찾되 목록에는 기술 ID를 표시하지 않으며, 긴 목록은 가상 렌더링합니다.

v0.19.2에서는 상위 레이어를 공통 폴더 아이콘으로 통일하고 국가 레이어 잠금을 열린·잠긴 자물쇠 아이콘으로 표시합니다. 체크박스는 선택 시 내부만 강조색으로 채우고 외곽선은 중립색을 유지합니다.

v0.19.1에서는 하단 상태 정보를 좌·중·우로 분산하지 않고 지도 가용 영역 왼쪽의 한 묶음으로 정렬했습니다. 하단바 배경과 콘텐츠 영역을 분리하고 과거 flex·grid 중복 규칙을 제거해 패널 개폐와 화면 폭 변화에도 정보 순서와 간격이 유지됩니다.

v0.18.1에서는 편집 대상 유형 배지를 완전히 제거하고, 패널 닫기 버튼의 여백과 수계 정보 카드의 중첩 테두리를 정리했습니다. v0.18.0에서 통합한 공용 UI 규격은 그대로 유지합니다.
...
- `지역`: 국가 국토를 구분하는 전용 영역입니다. 나누기·인접 지역 합치기·영역 다시 지정·다른 국가로 이전·독립 국가 전환을 지원합니다.
- `행정구역`: 국가·지역·상위 행정구역을 부모로 가지는 전용 영역입니다. 부모 관계에 따라 1급·2급 단계를 자동 계산하고 같은 부모·단계의 인접 구역만 합칠 수 있습니다.
- `민족·종교·언어`: 소유권과 분리된 주제 영역입니다. 원본 좌표는 보존하고 화면에서는 현재 육지 안에만 표시하며, 같은 역할끼리 나누거나 합칠 수 있습니다.
- `강·호수`: 수계 객체로 유지하며 국가 소유권이나 영토 연산에 참여하지 않습니다.

국가 Polygon이 국경과 해안선의 단일 기준입니다. 지역·행정구역 외곽은 따로 꼭짓점을 움직이지 않고 안전한 나누기·합치기·영역 다시 지정으로만 변경합니다. 부모별 영역을 만들기 시작하면 이름 없는 나머지 면적을 `미지정 지역` 또는 `미지정 행정구역`으로 유지해 국토에 구멍이 생기지 않게 합니다. 국가 합병·편입·분리 때 관계된 영역의 소속과 형상도 같은 실행취소 단위로 갱신됩니다.
