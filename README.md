...
v0.20.0에서는 지도와 연결된 레이어·편집 패널을 화면 폭과 관계없이 논모달로 통일했습니다. 모바일 패널은 기본·중간·최대 높이의 3단 스냅 바텀시트로 동작하며, 열린 상태에서도 지도 이동·확대·선택이 가능합니다. 파일 메뉴는 별도의 최상위 모달 오버레이로 분리해 편집 패널 위에서도 항상 조작할 수 있습니다.
