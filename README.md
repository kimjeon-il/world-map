...
작업공간은 화면 너비에 따라 세 단계로 전환됩니다. `1200px` 이상에서는 좌우 패널이 고정되고, `800~1199px`에서는 왼쪽 패널과 지도는 유지한 채 편집 패널이 서랍으로 열립니다. `799px` 이하에서는 지도가 전체 너비를 사용하며 도구·편집 패널은 하단 시트로 표시됩니다.
