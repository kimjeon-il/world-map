...
v0.24.0에서는 레이어 하위 항목의 장식 아이콘과 중복 `국명` 문구를 제거하고, 각 항목 오른쪽에 실행취소 가능한 삭제 버튼을 추가했습니다. 또한 지형지물 분류를 실제 객체 역할로 연결했습니다. 영토·행정구역은 소유 국가와 상위 영역, 해안선 결합 관계를 가지며 영역 나누기·동급 영역 합치기·국가 영토 반영·국가 전환을 지원합니다. 국가 해안선은 단일 기준 형상으로 유지되고 연결된 영토·행정구역이 함께 갱신되며, 민족·종교·언어 영역은 원본 형상을 유지한 채 육지 안에서만 표시됩니다. 이 관계 필드는 GeoJSON과 GeoPackage에도 보존됩니다.
...
- `영토`: 소유 국가의 영토 단위입니다. 나누기·같은 소유 국가의 영토와 합치기·국가 영토 반영·독립 국가 전환을 지원합니다.
- `행정구역`: 소유 국가와 선택적인 상위 영역을 가지며, 같은 소유 국가의 행정구역과 나누거나 합칠 수 있습니다.
...
국가 Polygon이 해안선의 단일 기준입니다. 영토·행정구역은 소유 국가를 지정하면 국가 해안선과 결합되며, 연결된 해안 꼭짓점은 영역에서 따로 움직이지 않고 `해안 구간 수정`으로 국가와 함께 변경합니다. 국가 합병·편입·분리 때 관계된 영역의 소유 국가와 형상도 같은 실행취소 단위로 갱신됩니다.

기존 GeoJSON·GeoPackage의 도형은 열 때 분류와 형상에 따라 자동으로 역할을 부여합니다. 관계는 `aw_role`, `aw_owner_id`, `aw_parent_id`, `aw_topology_group`, `aw_land_binding` 필드에 기록됩니다.
