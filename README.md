...
v0.14.0에서는 스크롤바가 나타날 때 패널 내용이 옆으로 밀리지 않도록 폭을 고정하고, 글꼴·컨트롤·카드의 위계를 하나의 UI 체계로 통합했습니다. 국가 합병은 대상 국가 선택 즉시 완료되며, 국가 추가와 영토 편입은 `국경선으로 나누기`와 `영토 조각 선택`을 명시적인 방식 탭으로 제공합니다.
