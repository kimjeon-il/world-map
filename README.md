...
정적 파일을 배포하기 전에는 `pnpm generate:build-meta`로 현재 앱 버전과 배포 build ID를 반영한 자산 메타데이터를 생성하고, `pnpm check:version`으로 HTML·스크립트·Worker·캐시 revision의 일치를 확인합니다.
...
v0.31.0에서는 지도·레이어·추가·편집 패널을 공통 Surface 셸과 탭 구조로 정리하고, 편집창에 정보·편집·관계 탭을 도입했습니다. 추가창은 만들기·라이브러리 경로를 유지하면서 공통 객체 분류와 Command Row 표현을 사용하며, build metadata로 앱·자산 버전을 일치시킵니다.
...
v0.28.0에서는 프로젝트 레이어와 분리된 국가·지역 라이브러리를 추가했습니다. 안정적 `libraryId`, 다국어 이름·별칭, 존속 기간, 시대별 `GeometryVersion`, 출처·불확실성을 검색·미리보기하고 독립 프로젝트 인스턴스로 추가할 수 있습니다. 현존 국가는 기존 Natural Earth 데이터를 어댑터로 공급하며, 과거 국가 시험 항목은 근사 경계·낮은 신뢰도를 명시합니다. `WorldSnapshot`은 원본을 변경하지 않는 템플릿으로 등록됩니다.
...
v0.24.0에서는 레이어 하위 항목의 장식 아이콘과 중복 `국명` 문구를 제거하고, 각 항목 오른쪽에 실행취소 가능한 삭제 버튼을 추가했습니다. 권역·행정구역은 기타 객체와 분리된 국가 영역으로 관리하며 국가별·행정 단계별 레이어, 전용 편집창, 나누기·합치기·영역 다시 지정·국가 이전·독립 작업을 제공합니다. 국가 Polygon은 국경과 해안선의 단일 기준이고, 명시적 구분을 시작한 부모의 남는 면적은 미지정 영역으로 보존됩니다. 이 관계는 프로젝트 상태와 GeoJSON·GeoPackage 입출력에 포함됩니다.
...
v0.13.2에서는 `보기`에 투영법과 지도 엔진만 남기고 지형 표시를 독립된 최상위 레이어 폴더로 분리했습니다. 폴더에서 `국가 색상 유지` 또는 `지형 높낮이 색상`을 선택하며, 국가 색상 유지의 명암은 일관된 기본값으로 표시합니다. 내장 강과 호수는 수동 색상 설정 없이 현재 표시 중인 바다 계열을 자동으로 따릅니다.

v0.13.1에서는 지형지물 하위 레이어를 `강`, `호수`, `기타 객체`와 객체 수만 표시하도록 간소화해 좁은 패널에서도 레이어명을 읽기 쉽게 만들었습니다. Hydro·Natural Earth 출처와 수계 준비 상태, 재시도 안내는 기존 상단 알림과 레이어 선택 안내에서 계속 제공합니다.
...
v0.12.6에서는 국가 외곽선에서 극점·날짜변경선용 인공 폐합 구간을 제외하고, 국가 채움용 Polygon은 그대로 보존합니다. `국가 색상 유지`는 국가 면 스텐실로 육지만 높낮이 명암을 더하며, `지형 높낮이 색상`은 강·호수가 없는 Natural Earth `HYP_HR_SR` 육지와 기존 해저 지형을 합성합니다. 강과 호수는 각각 흰색 또는 현재 테마의 바다색을 선택할 수 있습니다.
...
v0.12.0에서는 지형 음영과 Natural Earth 수계를 도입하고, 필요한 화면 단계의 타일만 불러오는 비차단 물리 데이터 로딩을 추가했습니다.

v0.11.2에서는 지도·레이어·편집 패널의 작업 흐름과 상태 안내를 정리하고, 강·호수 생성과 지형지물 레이어 표시를 추가했습니다.

v0.11.0에서는 화면 너비에 따라 패널과 편집창이 전환되는 반응형 작업공간을 도입했습니다.

v0.10.3에서는 운영체제·브라우저의 `prefers-color-scheme`에 따른 밝은 테마와 어두운 테마를 지원했습니다.

v0.10.2에서는 레이어 폴더와 국가 GPU 메시를 추가해 객체 탐색과 WebGL 렌더링 기반을 마련했습니다.

v0.10.0에서는 QGIS 벡터 파일과 GeoPackage를 가져오고 저장하는 GIS 교환 기능을 추가했습니다.
...
레이어 패널은 `영토·구역`, `인문 분포`, `지형지물`로 구성됩니다. 강, 호수, 기타 객체 폴더를 펼쳐 개별 항목을 표시하거나 숨길 수 있으며, 국가명과 지명 표시는 `보기 > 이름 표시`에서 조절합니다. 권역과 행정구역은 국가별로 묶이고 행정구역은 다시 단계별로 표시됩니다. 내장 수계는 잠겨 있으며 선택 후 `편집용 복사 만들기`를 사용하면 원본을 숨기고 같은 형상의 편집 객체를 만들 수 있습니다. 개별 표시 상태와 폴더 열림 상태는 자동저장 및 GeoPackage의 `pandolab_project_settings`에 보존됩니다.
...
보기 패널에서 `국가 색상 유지`와 `지형 높낮이 색상`을 전환할 수 있습니다. 국가 색상 유지 방식의 높낮이 명암은 일관된 기본값으로 표시합니다. 지형은 Natural Earth raster 3.2.0의 `GRAY_HR_SR_OB`, 무수계 `HYP_HR_SR`, 해저 지형용 `HYP_HR_SR_OB_DR` 21,600×10,800 원본을 결합한 무손실 WebP 타일 피라미드입니다. RGB에는 강·호수가 중복되지 않는 육지 지형색과 해저 수심을, 알파 채널에는 중립 명암을 넣습니다. 초기 국가지도 로딩을 막지 않으며 현재 화면에 필요한 단계의 타일만 불러옵니다.
...
- `권역`: 국가 국토를 구분하는 전용 영역입니다. 나누기·인접 권역 합치기·영역 다시 지정·다른 국가로 이전·독립 국가 전환을 지원합니다.
- `행정구역`: 국가·권역·상위 행정구역을 부모로 가지는 전용 영역입니다. 부모 관계에 따라 1급·2급 단계를 자동 계산하고 같은 부모·단계의 인접 구역만 합칠 수 있습니다.
...
국가 Polygon이 국경과 해안선의 단일 기준입니다. 권역·지방·행정구역 외곽은 따로 꼭짓점을 움직이지 않고 안전한 나누기·합치기·영역 다시 지정으로만 변경합니다. 부모별 영역을 만들기 시작하면 이름 없는 나머지 면적을 `미지정 지역` 또는 `미지정 행정구역`으로 유지해 국토에 구멍이 생기지 않게 합니다. 국가 합병·편입·분리 때 관계된 영역의 소속과 형상도 같은 실행취소 단위로 갱신됩니다.
...
- `places`: 지명 Point
- `generic_features_point`, `generic_features_line`, `generic_features_polygon`: 기타 객체의 Point·Line·Polygon geometry
- `territories`: 국가별 권역 MultiPolygon과 소속·상태 관계
...
내장 전 세계 수계는 파일 용량이 중복되지 않도록 GeoPackage에 넣지 않습니다. 사용자가 만든 강·호수와 `편집용 복사`로 만든 객체는 canonical 수계 테이블에 저장되고 기타 객체는 `generic_features_*` 테이블에 저장됩니다. QGIS에서는 이 사용자 벡터를 일반 레이어로 열어 편집할 수 있고, 판도연구소로 다시 열면 국기·메모·화면 상태와 내장 수계 버전도 복원됩니다. 자동저장은 별도의 판도연구소 전용 IndexedDB에만 저장되며 공개 프로젝트 파일로 노출되지 않습니다.
