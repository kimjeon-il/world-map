...
test('retired DOM hooks stay absent and every app module uses the current revision', async ({ page }) => {
  ...
  expect(audit.moduleUrls.every(url => new URL(url).searchParams.get('v') === '0.30.0-r35')).toBe(true);
  expect(errors).toEqual([]);
});
...
test('country edit worker executes annex, new-country, merge, commit, discard, and failure paths', async ({ page }) => {
  ...
  const worker = new Worker('/assets/js/workers/map-edit-worker.js?v=0.30.0-r35');
    let workerError = '';
...
test('river territory partition Worker returns disjoint donor cells', async ({ page }) => {
  await page.goto('/');
  const result = await page.evaluate(async () => {
    const worker = new Worker('/assets/js/workers/river-territory-partition-worker.js?v=0.30.0-r35', { type: 'module' });
    const donor = {
