...
test('GIS export shows one subunit choice and counts subunit-only selection', async ({ page }) => {
  ...
  for (const input of await page.locator('.gis-export-layers input:enabled').all()) await input.uncheck();
  await page.locator('.gis-export-layers input[value="subunits"]').check();

  await page.locator('.gis-export-layers').dispatchEvent('change');
  await expect(page.locator('#gisExportModal')).toContainText(/하위단위.*47개/);
