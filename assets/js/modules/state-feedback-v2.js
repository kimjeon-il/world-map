...
function installStyle(documentRef) {
  if (documentRef.querySelector('link[data-pandolab-ui-v2="features-state-feedback"]')) return;
  const href = new URL('../../css/features/state-feedback.css', import.meta.url);
  const revision = String(globalThis.PANDOLAB_ASSET_REVISION || '').trim();
  if (revision) href.searchParams.set('v', revision);
  const link = documentRef.createElement('link');
  link.rel = 'stylesheet';
  link.href = href.href;
  link.dataset.pandolabUiV2 = 'features-state-feedback';
...
function installProgressSemantics(documentRef) {
  const targets = [
    documentRef.querySelector('#gisImportProgress .ui-progress'),
  ].filter(Boolean);
