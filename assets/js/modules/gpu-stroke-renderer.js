...
function createProgram(device) {
  ...
  const vertexSource = version === 2 ? `#version 300 es
    ...
    vec3 project(vec2 coord){float lon=coord.x*0.017453292519943;float lat=coord.y*0.017453292519943;vec2 p;float depth;
      ...
      else{float my=log(tan(0.7853981633974483+clamp(lat,-1.4844222297453324,1.4844222297453324)*0.5));float cy=log(tan(0.7853981633974483+clamp(uFlatCenter.y,-1.4844222297453324,1.4844222297453324)*0.5));p=uTranslate+uScale*vec2(lon+uWorldOffset-uFlatCenter.x,-(my-cy));depth=1.0;}return vec3(p,depth);}
