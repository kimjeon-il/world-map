export function createCoastReconciliationController({ document, window, elements, onResolve } = {}) {
  ...
  function open({ adminName = '행정구역', countryName = '국가', conflicts = [] } = {}) {
    if (!elements.modal) return Promise.resolve({ direction: 'cancel' });
    close('cancel');
    elements.title.textContent = '해안선이 서로 일치하지 않습니다';
    elements.message.textContent = `${adminName}과(와) ${countryName}의 해안선이 같은 구간에서 서로 다릅니다.\n어느 해안선을 기준으로 맞출지 선택하세요.`;
    const impactItems = [
  ...
  function bind() {
    ...
    elements.admin?.addEventListener('click', () => close('admin-to-country'));
