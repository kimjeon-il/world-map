  ...
  loader.onmessage = event => {
    ...
    if (data.type === 'mesh-ready') {
      ...
      startupGate.queue('apply-mesh', () => {
        resolveMesh({ meshBuffer: data.meshBuffer, preparedStroke: data.preparedStroke, spatialBlocks: data.spatialBlocks, identity: data.identity || null, metrics: data.metrics || null });
